...
const DARK_BACKGROUND = [0.043, 0.059, 0.098];
const LIGHT_BACKGROUND = [0.965, 0.973, 0.984];

function getBackgroundColor(darkMode: boolean) {
...
export async function prepareFluid(
  ...
  resizeFluid(fluid, output, darkMode);
  await fluid.passes.display.compile({ colors: [output.format] });
}

export function resizeFluid(
  fluid: Fluid,
  output: Target,
  darkMode = true
): void {
  fluid.passes.display.set({
    config: {
      ...
      background_color: getBackgroundColor(darkMode),
    },
  });
}

export function setFluidTheme(
  fluid: Fluid,
  darkMode: boolean
): void {
  fluid.passes.display.set({
